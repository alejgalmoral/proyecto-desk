...
void agregarEquipo(instrumento *&equipos, int &cant, instrumento nuevo) {
	instrumento *nuevoArreglo = new instrumento[cant + 1];
    
	instrumento *ptrOrig = equipos;
	instrumento *ptrDest = nuevoArreglo;

	// Copia profunda de los elementos existentes mediante aritmetica de punteros
	for (int i = 0; i < cant; i++) {
		ptrDest->codigo = ptrOrig->codigo;
        
		asignarCadena(ptrDest->nombre, ptrOrig->nombre);
		asignarCadena(ptrDest->laboratorio, ptrOrig->laboratorio);
		asignarCadena(ptrDest->tipo, ptrOrig->tipo);
        
		strcpy(ptrDest->estado, ptrOrig->estado);
		ptrDest->costo = ptrOrig->costo;
		ptrDest->semestre = ptrOrig->semestre;
        
		asignarCadena(ptrDest->desck, ptrOrig->desck);
		ptrDest->horas_a = ptrOrig->horas_a;

		// Liberar memoria dinamica previa
		delete[] ptrOrig->nombre;
		delete[] ptrOrig->laboratorio;
		delete[] ptrOrig->tipo;
		delete[] ptrOrig->desck;

		ptrOrig++;
		ptrDest++;
	}

	// Copia del nuevo elemento
	ptrDest->codigo = nuevo.codigo;
	asignarCadena(ptrDest->nombre, nuevo.nombre);
	asignarCadena(ptrDest->laboratorio, nuevo.laboratorio);
	asignarCadena(ptrDest->tipo, nuevo.tipo);
	strcpy(ptrDest->estado, nuevo.estado);
	ptrDest->costo = nuevo.costo;
	ptrDest->semestre = nuevo.semestre;
	asignarCadena(ptrDest->desck, nuevo.desck);
	ptrDest->horas_a = nuevo.horas_a;

	// Liberar memoria de la variable temporal
	delete[] nuevo.nombre;
	delete[] nuevo.laboratorio;
	delete[] nuevo.tipo;
	delete[] nuevo.desck;
...
void agregarUsuario(usuario *&estudiantes, int &cant, usuario nuevo) {
	usuario *nuevoArreglo = new usuario[cant + 1];
	...
	for (int i = 0; i < cant; i++) {
		...
		asignarCadena(ptrDest->nomes, ptrOrig->nomes);
		asignarCadena(ptrDest->programa, ptrOrig->programa);
        
		ptrDest->semes = ptrOrig->semes;
		ptrDest->pena_a = ptrOrig->pena_a;
		ptrDest->cont_s = ptrOrig->cont_s;

		delete[] ptrOrig->nomes;
		delete[] ptrOrig->programa;
	...
	ptrDest->semes = nuevo.semes;
	ptrDest->pena_a = nuevo.pena_a;
	ptrDest->cont_s = nuevo.cont_s;

	delete[] nuevo.nomes;
	delete[] nuevo.programa;
	...
	if (des == 1) {
		...
		while (togore.getline(linea, 300)) {
			...
			while (token != NULL) {
				...
				case 4: strcpy(esta_aux, token); break;
				case 5: costo_aux = atoi(token); break;
				case 6: sme_aux = atoi(token); break;
...
void TVT(usuario *&estudiantes, int &cant_actual) {
	...
	if (des == 1) {
		...
		for (int y = 0; y < dan; y++) {
			...
			cin.ignore();
			cin.getline(nom_aux, 300);
			asignarCadena(temp.nomes, nom_aux);

			cout << "ingresa programa del estudiante " << y + 1 << endl;
...
int main() {
	...
	do {
		...
		switch (D4C) {
		case 1:
			TNT(equipos, eqe); // Pasa eqe por referencia
			break;
		case 2:
			TVT(estudiantes, usu); // Pasa usu por referencia
			break;
		case 4:
